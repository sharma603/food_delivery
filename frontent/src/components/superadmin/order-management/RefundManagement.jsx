...
const RefundManagement = ({ user, onLogout }) => {
  ...
  const mockRefunds = [
    ...
    },
    {
      ...
      requestedAt: '2024-01-14T12:20:00Z',
      processedAt: '2024-01-14T13:30:00Z',
      paymentMethod: 'Credit Card'
    }
  ];
  ...
  const fetchRefunds = useCallback(async () => {
    try {
      ...
      setTimeout(() => {
        ...
        setTotalRefunds(mockRefunds.length);
        setTotalPages(1);
        setLoading(false);
      }, 1000);
    } catch (error) {
  ...
  const fetchRefundStats = useCallback(async () => {
    try {
      ...
      const stats = {
        ...
        processedRefunds: mockRefunds.filter(r => r.status === 'completed').length,
        totalAmount: mockRefunds.reduce((sum, refund) => sum + refund.refundAmount, 0)
      };
