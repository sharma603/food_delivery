...
const AllMenuItems = () => {
 ...
 .filter(item => {
    ...
    // Get category name, handling ObjectId strings
    const itemCategoryName = (() => {
      if (typeof item.category === 'object' && item.category !== null) {
        return item.category?.displayName || item.category?.name || 'Uncategorized';
      }
      // If category is a string that looks like ObjectId (24 hex characters), treat as Uncategorized
      if (typeof item.category === 'string' && item.category.match(/^[0-9a-fA-F]{24}$/)) {
        return 'Uncategorized';
      }
      // If category is a valid string name, use it
 ...
 <div key={item.id} className="bg-white rounded-lg shadow-sm border border-gray-100 hover:shadow-md transition-shadow w-full max-w-full sm:w-full md:w-[250px] h-auto min-h-[300px] mx-auto flex flex-col">
 {/* Item Image */}
