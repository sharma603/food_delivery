...
import { Search, Filter, ToggleLeft, ToggleRight, Clock, AlertTriangle } from 'lucide-react';
...
const StatusControl = () => {
  ...
  const [selectedRestaurants, setSelectedRestaurants] = useState([]); // Reserved for future bulk operations
  ...
  useEffect(() => {
    const fetchRestaurants = async () => {
      try {
        ...
        setRestaurants([]); // Empty for now - will be populated from API
      } catch (error) {
  ...
  // Use API data when available, otherwise use empty array
  const restaurantList = restaurants.length > 0 ? restaurants : [];
