...
const SuperAdminVerifyOTP = () => {
  ...
  const handleVerifyOTP = async (otpValue) => {
    ...
    try {
      const response = await fetch(`${AppConfig.API.BASE_URL}/auth/superadmin/verify-otp`, {
        method: 'POST',
        headers: {
  ...
  const handleResendOTP = async () => {
    ...
    try {
      const response = await fetch(`${AppConfig.API.BASE_URL}/auth/superadmin/forgot-password`, {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
            ...
            <div className="flex justify-center gap-1.5 sm:gap-2 px-2">
              {otp.map((digit, index) => (
                <input
                  ...
                  className={`w-10 h-12 sm:w-12 sm:h-14 text-center text-lg sm:text-xl font-bold border-2 rounded-lg transition-all focus:outline-none focus:ring-2 focus:ring-indigo-500 ${
                    digit 
            ...
            <div className="text-center">
              {canResend ? (
                <button
                  ...
                  className="text-xs sm:text-sm text-indigo-600 hover:text-indigo-500 font-medium transition-colors disabled:opacity-50"
                >
