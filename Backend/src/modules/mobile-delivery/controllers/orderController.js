...
export const updateDeliveryStatus = async (req, res) => {
  try {
    ...
    // Validate status transitions for picked_up
    if (status === 'picked_up') {
      // Can mark as picked_up from ready or confirmed status (after acceptance)
      if (!['ready', 'confirmed'].includes(order.status)) {
        return res.status(400).json({
          success: false,
          message: `Order must be in 'ready' or 'confirmed' status to be marked as picked up. Current status: ${order.status}`
        });
      }
    }
    ...
    if (status === 'delivered' && order.deliveryPerson) {
      try {
        ...
        if (deliveryPerson) {
          ...
          deliveryPerson.totalEarnings = (deliveryPerson.totalEarnings || 0) + deliveryFee;
        ...
        console.error('Error updating delivery person earnings:', earningsError);
      }
    }
...
export const acceptOrder = async (req, res) => {
  try {
    ...
    // Keep status as 'ready' when accepted - delivery person will mark as 'picked_up' when they reach restaurant
    // Don't change status to 'confirmed' - keep it as 'ready' so they can mark it as picked_up
    order.trackingUpdates = [
      ...(order.trackingUpdates || []),
      {
        status: 'assigned',
        timestamp: new Date(),
        message: `Order accepted by delivery person. Proceed to restaurant for pickup.`,
        deliveryPerson: deliveryPersonId
      }
    ];
